...
describe('gl-costants', () => {
  ...
  describe('GL_CONSTANTS enum', () => {
    it('constains constants', () => {
      expect(GL_CONSTANTS.COLOR_CLEAR_VALUE).to.equal(0x0c22)
      expect(GL_CONSTANTS.CW).to.equal(0x0900)
      expect(GL_CONSTANTS.FRAMEBUFFER_ATTACHMENT_TEXTURE_CUBE_MAP_FACE).to.equal(0x8cd3)
    })

    it('values match chrome 91 constants', () => {
      for (const [k, v] of Object.entries(getChromeConstants())) {
        expect(GL_CONSTANTS[k as any]).to.equal(v)
      }
    })
  })

  describe('GL_CONSTANTS_NAMES', () => {
    ...
    it('has all values in GL_CONSTANTS', () => {
      expect(GL_CONSTANTS_NAMES.filter((x) => typeof GL_CONSTANTS[x] === 'number'))
    })
  })
  ...
  describe('glConstantGetName', () => {
    ...
    it('returns constant names by hex number string', () => {
      expect(glConstantGetName('0x100')).to.equal('DEPTH_BUFFER_BIT')
      expect(glConstantGetName('0x00000100')).to.equal('DEPTH_BUFFER_BIT')
      expect(glConstantGetName('  0x8212  ')).to.equal('FRAMEBUFFER_ATTACHMENT_RED_SIZE')
    })
    ...
    it('returns undefined if no constant found', () => {
      expect(glConstantGetName('DEPTH_BUFFER_BITXXX')).to.equal(undefined)
      expect(glConstantGetName(12345123)).to.equal(undefined)
    })
  })
})
